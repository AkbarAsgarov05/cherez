...
// ✅ Vite üçün düzgün istifadə - import.meta.env
...
// İstifadəçi tərəfindən mesaj göndərmək (public)
export const sendContactMessage = async (formData) => {
  try {
    const response = await api.post('/messages/contact', {
      ...
      subject: formData.subject || 'Əlaqə formu',
      message: formData.message
    });
    return response.data;
  } catch (error) {
    console.error('Mesaj göndərərkən xəta:', error);
    return error.response?.data || { success: false, message: 'Xəta baş verdi' };
  }
};
